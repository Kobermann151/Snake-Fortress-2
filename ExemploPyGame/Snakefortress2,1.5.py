...
IMG_CABECA_P1       = carregar_imagem("imagens/HeavyFace2.png", GRID)
IMG_CABECA_P2       = carregar_imagem("imagens/Soldier2.png", GRID)
IMG_CABECA_P1_MORTE = carregar_imagem("imagens/Heavyded.png", GRID)
IMG_CABECA_P2_MORTE = carregar_imagem("imagens/cabeca_p2_morte.png", GRID)
IMG_BOMBA           = carregar_imagem("imagens/Pumpkinbomb.png", GRID)
IMG_FUNDO_MENU       = carregar_fundo("imagens/menu_fundo.jpg", LARGURA, ALTURA)
...
IMAGENS_FRUTAS = [
    carregar_imagem(f"imagens/fruta{i}.png", GRID) for i in range(1,6)
]
...
while True:
    ...
    if estado == MENU:
        ...
        titulo = fonte_g.render("SNAKE FORTRESS 2", True, AMARELO)
    ...
    if estado == JOGANDO:
        ...
        if x1 < 0 or x1 >= LARGURA or y1 < 60 or y1 >= ALTURA:
            vencedor = "SOLDIER VENCE" if p2 is not None else "FIM DE JOGO"
            p1.morta = True
            estado = FIM
